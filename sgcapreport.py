...
os.environ['SYMCLI_DB_FILE'] = os.path.abspath(args.symapi)
...
for elem in sgtree.iterfind('SG'):
    ...
    for member in elem.iterfind('DEVS_List/Device'):
        ...
        if sgname not in sgcapacity:
            # Create new dict key and list (i.e. we just found the meta head)
            sgcapacity[sgname] = devcapacity
        else:
            # Append to existing key and list (i.e. we found a meta member)
            sgcapacity[sgname] += devcapacity
    ...
    for cascadesg in elem.iterfind('SG_Info/SG_group_info/SG'):
        if cascadesg.find('Cascade_Status').text == "IsChild":
            if sgname not in sgchildren:
                sgchildren[sgname] = [cascadesg.find('name').text]
            else:
                sgchildren[sgname].append(cascadesg.find('name').text)
        elif cascadesg.find('Cascade_Status').text == "IsParent":
            if sgname not in sgparents:
                sgparents[sgname] = [cascadesg.find('name').text]
            else:
                sgparents[sgname].append(cascadesg.find('name').text)
